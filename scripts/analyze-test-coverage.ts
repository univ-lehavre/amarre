...
class TestCoverageAnalyzer {
  ...
  analyzeCoverage(): void {
    ...
    if (total.branches.pct < this.thresholds.branches) {
      failures.push(`Branches coverage (${total.branches.pct}%) below threshold (${this.thresholds.branches}%)`);
    }
    if (total.functions.pct < this.thresholds.functions) {
      failures.push(`Functions coverage (${total.functions.pct}%) below threshold (${this.thresholds.functions}%)`);
    }
    if (total.lines.pct < this.thresholds.lines) {
      failures.push(`Lines coverage (${total.lines.pct}%) below threshold (${this.thresholds.lines}%)`);
    }
