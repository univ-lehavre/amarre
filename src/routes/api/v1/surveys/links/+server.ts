...
export const _openapi = {
  ...
  responses: {
    ...
    400: { description: 'Paramètres manquants', content: { 'application/json': { schema: surveyLinkResponse } } },
    401: { description: 'Non authentifié', content: { 'application/json': { schema: surveyLinkResponse } } },
    422: { description: 'Lien invalide', content: { 'application/json': { schema: surveyLinkResponse } } },
...
export const GET: RequestHandler = async ({ locals, fetch, url }) => {
  try {
    ...
    if (!record || !instrument) {
      return json(
        { data: null, error: { code: 'bad_request', message: 'Missing record or instrument parameter' } },
        { status: 400 },
      );
    }
