...
const buildName = (data: EAV[]): string => {
  const first_name = data.filter(record => record.field_name === 'first_name').map(record => record.value)[0];
  const middle_name = data.filter(record => record.field_name === 'middle_name').map(record => record.value)[0];
  const last_name = data.filter(record => record.field_name === 'last_name').map(record => record.value)[0];
...
export { buildName, getID, capitalizeFirstLetter, transformToName };
