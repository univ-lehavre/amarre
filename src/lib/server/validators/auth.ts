...
export const validateMagicUrlLogin = (userId?: unknown, secret?: unknown): { userId: string; secret: string } => {
  if (!userId || !secret) throw new MagicUrlLoginValidationError('Login failed', { cause: 'Missing userId or secret' });
  if (typeof userId !== 'string' || typeof secret !== 'string')
    throw new MagicUrlLoginValidationError('Login failed', { cause: 'userId and secret must be strings' });
  if (!isHexadecimal(userId) || !isHexadecimal(secret))
    throw new MagicUrlLoginValidationError('Login failed', { cause: 'Invalid userId or secret format' });
...
export const validateUserId = (userId?: unknown): string => {
  ...
  if (!isHexadecimal(userId)) throw new UserIdValidationError('Operation failed', { cause: 'Invalid userId format' });
  return userId;
};
