...
export const ApiError = z
  .object({ code: z.string(), message: z.string(), details: z.unknown().optional(), cause: z.unknown().optional() })
  .strict()
  .openapi('ApiError');

export const PaginationMeta = z
  .object({ page: z.number().int().min(1), per_page: z.number().int().min(1), total: z.number().int().nonnegative() })
  .strict();
