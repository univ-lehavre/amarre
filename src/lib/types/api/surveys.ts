...
export const surveyRequestItem = z
  .object({
    ...
    invitation_type: z.string().describe('Type d’invitation (ex: "Recherche", "Enseignement", "Les deux")'),
    invite_nom: z.string().describe('Nom complet du chercheur invité'),
    mobilite_universite_eunicoast: z.string().describe('Université de destination dans le réseau EUNICoast'),
    mobilite_universite_gu8: z.string().describe('Université de destination dans le réseau GU8'),
    mobilite_universite_autre: z.string().describe("Nom de l'université de destination (hors réseaux)"),
